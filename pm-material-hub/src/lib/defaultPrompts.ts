...
7. 每张卡必须有 evidence_chunk_ids。缺失字段用空字符串或空数组。除 JSON 外不得输出任何文字。`,
  ...
  '03_': `你是工业自动化技术手册分析专家。输入是技术手册 raw JSON，其中 chapters 表示按章节压缩后的原始内容。03 手册不是按 MLFB 组织的资料，禁止强行按 MLFB 建卡。
...
规则：
1. 以章节主题为卡片粒度，一章或一组相邻小节生成一张技术主题卡。
2. 不要为每个 MLFB 建卡；MLFB 只作为 related_mlfbs 关联标签。
3. 系统级内容必须保留为系统级技术卡，例如安装、接线、组态、调试、诊断、维护、安全、系统限制、技术规范。
...
7. 忽略目录、页眉页脚、版权、警告模板重复内容和空白占位。
8. 每张卡必须有 chapter_ids 和 evidence_chunk_ids。除 JSON 外不得输出任何文字。`,
